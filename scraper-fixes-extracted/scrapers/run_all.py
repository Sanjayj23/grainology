...
# run fetch_indiadataportal() as a separate, occasional job.
...
# Sources whose failure should make the GitHub Actions run fail loudly
# (red X, notification). Everything else is best-effort: a fragile
# government website having a bad day is expected and shouldn't page you.
CRITICAL_SOURCES = {"datagovin"}
...
def main() -> None:
    ...
    # Only fail the GitHub Actions run (red X / notification) if a CRITICAL
    # source produced nothing. Best-effort sources (eNAM, Agmarknet direct)
    # are expected to occasionally have a bad day against fragile, frequently
